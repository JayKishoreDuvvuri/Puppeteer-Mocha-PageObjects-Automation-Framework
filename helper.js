...
export default class Launcher {
  ...
  async getAttributeValue(selector) {
    await this.page.waitForSelector(selector);
    let value = await this.page.$eval(selector, (element) =>
      element.getAttribute("value")
    );
    return value;
  }

  async setFromDatePicker() {
    await this.page.evaluate(() =>
      document.querySelector("#datefrom").setAttribute("value", "2020-12-18")
    );
  }

  async setToDatePicker() {
    await this.page.evaluate(() =>
      document.querySelector("#dateto").setAttribute("value", "2020-12-19")
    );
  }
